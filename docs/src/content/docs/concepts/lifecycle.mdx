...
<Callout>
  If a hook returns a value other than `undefined` typically a `Response` built via `json` the lifecycle short-circuits: no
  further hooks or the handler run, and that value is sent to the client as-is. This is how the `onRequest` example below rejects
  unauthenticated requests before routing even happens.
</Callout>
