...
- Added the `handle()` function to `createRouter()` for handling incoming requests through a single universal request handler. The `handle()` function supports any incoming request and is returned directly by `createRouter()`. [#67](https://github.com/aura-stack-ts/router/pull/67)
